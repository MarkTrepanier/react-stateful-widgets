...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    programmers.forEach(programmer =>{
      if(programmer.id == featuredProgrammer){
        console.log(programmer.name);
        console.log(featuredProgrammer)
        const cat = programmer.name;
        return cat;
    }
    else{
      return 'farts'
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredProgrammer != null ? 'gold' :'royalblue'// 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' id={dev.id}>
